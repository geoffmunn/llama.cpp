...
// HIFI: global imatrix guidance state (set once per model quantization run)
...
static float compute_model_params_b(const llama_hparams & hparams, int64_t n_vocab) {
    ...
    const int64_t attn_params = 4LL * n_embd * n_embd * n_layer;
    const int64_t ffn_params  = 3LL * n_embd * n_ff   * n_layer;
    const int64_t emb_params  = 2LL * n_vocab * n_embd;
    return (float)(attn_params + ffn_params + emb_params) / 1e9f;
}

// Q4_K_HIFI: type for critical tensors (output, embeddings, early attn_v)
...
// Q5_K_HIFI: type for critical tensors
...
// Fraction of early attn_v layers to upgrade for Q4_K_HIFI
...
// Fraction of early attn_v layers to upgrade for Q3_K_HIFI
...
// Fraction of early attn_v layers to upgrade for Q5_K_HIFI
...
// What type to use for enhanced attn_v in Q3_K_HIFI
static ggml_type get_q3_hifi_attn_v_type(float model_params_b) {
    if (model_params_b <= 5.0f) return GGML_TYPE_Q5_K_HIFI_RES8;
    return GGML_TYPE_Q6_K_HIFI_RES8;
}

// Parse blk.N. layer index from tensor name; returns -1 on failure
static int parse_layer_idx(const char * name) {
    int idx = -1;
    sscanf(name, "blk.%d.", &idx);
    return idx;
}
...
static ggml_type llama_tensor_get_type_impl(quantize_state_impl & qs, ggml_type new_type, const ggml_tensor * tensor, llama_ftype ftype, tensor_category category) {
    ...
    // HIFI / LITE ftype-specific overrides
    ...
    if (ftype == LLAMA_FTYPE_MOSTLY_Q4_K_HIFI) {
        if (category == tensor_category::OUTPUT ||
            ...
            if ((float)qs.i_attention_wv <= (float)qs.n_attention_wv * threshold) {
                new_type = get_hifi_enhanced_type(model_params_b);
        ...
        if (category == tensor_category::OUTPUT ||
            ...
            float threshold = get_q5_hifi_attn_v_threshold(model_params_b);
            if ((float)qs.i_attention_wv <= (float)qs.n_attention_wv * threshold) {
                new_type = get_q5_hifi_enhanced_type(model_params_b);
            } else if (use_more_bits(qs.i_attention_wv, qs.n_attention_wv)) {
                new_type = GGML_TYPE_Q6_K;
        ...
        // Other tensors remain Q2_K_HIFI (the default from llama_ftype_get_default_type)
    }
    ...
    for (size_t i = 0; i < tensors.size(); ++i) {
        ...
        if (params->dry_run) {
            ...
            if (!quantize) {
                ...
                // Build HIFI context for types that need it
                const float model_params_b = compute_model_params_b(qs.model.hparams, qs.model.vocab.n_tokens());
                const int   n_layers       = (int)qs.model.hparams.n_layer;
                ggml_hifi_quant_context hifi_ctx_val = {};
                ...
                if (new_type == GGML_TYPE_Q3_K_HIFI && ftype == LLAMA_FTYPE_MOSTLY_Q3_K_HIFI) {
                    ...
                    hifi_ctx_val = { base_outliers, tensor_importance, -1, n_layers, 1, model_params_b };
                    hifi_ctx_ptr = &hifi_ctx_val;
                    ...
                    hifi_ctx_val = { outlier_count, layer_importance, layer_idx, n_layers, 1, model_params_b };
                    hifi_ctx_ptr = &hifi_ctx_val;
                    ...
                    if (imatrix) {
                        ...
                        if (importance > g_importance_threshold) {
