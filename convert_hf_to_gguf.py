...
@ModelBase.register("Qwen3ForGuardModel")
class Qwen3GuardModel(Qwen3Model):
    model_arch = gguf.MODEL_ARCH.QWEN3GUARD

    def modify_tensors(self, data_torch: torch.Tensor, name: str, bid: int | None) -> Iterable[tuple[str, torch.Tensor]]:
        # Handle guard model specific tensors - these are classification/guard specific layers
        # that are not part of the base language model architecture
        if any(guard_keyword in name for guard_keyword in [
            "category_head",
            "query_category_head", 
            "risk_level",
            "category_layernorm",
            ...
            # Skip guard model specific tensors as they're not needed for the base model
            return []
        
        # For all other tensors, use the parent class mapping
        return super().modify_tensors(data_torch, name, bid)
