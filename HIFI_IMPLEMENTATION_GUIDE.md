...
Q3_K_HIFI and Q4_K_HIFI both have **native MMVQ vec_dot kernels** for single-token
inference (batch size ≤ `MMVQ_MAX_BATCH_SIZE`) as well as dequantization kernels for
`get_rows`, `to_float`, and large-batch cuBLAS fallback paths. The implementation
covers three areas:

#### 11.2.1 GPU Dequantization Kernel (`convert.cu`)

**Struct accessibility**: Despite the `#if !defined(GGML_COMMON_DECL_CUDA)` guards in
`ggml-common.h`, those guards wrap only the `#pragma pack` directives — the struct
`typedef` itself is always compiled. HIFI block structs are therefore fully accessible
inside CUDA/HIP translation units via normal struct-pointer casts.

Use a struct-pointer cast and access fields directly:

```c
// Q3_K_HIFI dequant kernel (64 threads, one CUDA block per Q3_K_HIFI block)
...
static __global__ void dequantize_block_q3_k_hifi(const void * __restrict__ vx,
    ...
    // ... standard Q3_K 64-thread dequant using q3k-> fields ...
...
**Q4_K_HIFI CUDA dequant** — identical approach with struct pointer. Neither Q3_K_HIFI
nor Q4_K_HIFI has an `outlier_count` field (only `block_q3_k_hifi_res8` does). Use
the FP16-zero sentinel `__half_as_ushort(x[i].outliers[k]) == 0` for both types.
Q4_K_HIFI also uses a 32-thread dequant (matching Q4_K), not 64 threads.

> **Critical pitfall — sentinel check**: The sentinel is `outlier_idx[k] = 255`,
> `outliers[k] = FP16(0.0f) = 0x0000`. The guard `if (idx >= 256) break` is
> **always false** when `idx` is stored as `int` with value 255 (255 < 256).
> Always use `__half_as_ushort(x[i].outliers[k]) == 0` to detect unused slots.

> **Note on `Q3_K_HIFI_MAX_OUTLIERS`**: This constant is defined in
> `ggml-quants-hifi.h`, which CUDA files do not include. Use `Q3_K_HIFI_OUTLIERS`
> (from `ggml-common.h`) as the loop bound instead — they are identical (both = 8).

#### 11.2.2 MMVQ Registration (`vecdotq.cuh` / `mmvq.cu`)

Both Q3_K_HIFI and Q4_K_HIFI have native MMVQ kernels. There is **no**
`ggml_cuda_has_mmvq_kernel` function in this codebase — `use_mul_mat_vec_q` uses the
plain `ggml_is_quantized(src0->type)` check, and all registered HIFI types are
quantized types. Do not add such a guard function.

To add a new HIFI MMVQ kernel, four changes are needed:

**1. `vecdotq.cuh`** — add a VDR macro and `vec_dot_*_q8_1` device function:
```c
// VDR: usually same as the base type
...
static __device__ __forceinline__ float vec_dot_q3_k_hifi_q8_1(
    ...
    // ... compute Q3_K base dot using vec_dot_q3_K_q8_1_impl_mmvq ...

    // Outlier correction: additive (Q3K zero-level identity means base = 0 at those pos)
    for (int k = 0; k < Q3_K_HIFI_OUTLIERS; ++k) {
...
> **Q4_K_HIFI note**: The Q4_K base contributes a non-zero value at zeroed outlier
> positions (due to the Q4K `dmin` offset). The correction in `vec_dot_q4_k_hifi_q8_1`
> currently applies `sum += outlier_val * q8_val * d8` (purely additive), which
> double-counts the Q4K base contribution at those positions. The fully correct
> formula is `sum += (outlier_val - q4k_decoded_at_pos) * q8_val * d8`, but this
> requires unpacking per-position scales, which is expensive in a GPU kernel. In
> practice the error is small when base-Q4K inlier positions decode close to 0.

**2. `mmvq.cu` — `get_vec_dot_q_cuda`**:
```c
case GGML_TYPE_Q3_K_HIFI: return vec_dot_q3_k_hifi_q8_1;
```

**3. `mmvq.cu` — `get_vdr_mmvq`**:
```c
case GGML_TYPE_Q3_K_HIFI: return VDR_Q3_K_HIFI_Q8_1_MMVQ;
```

**4. `mmvq.cu` — `mul_mat_vec_q_switch_type`**:
```c
case GGML_TYPE_Q3_K_HIFI:
    mul_mat_vec_q<..., vec_dot_q3_k_hifi_q8_1, VDR_Q3_K_HIFI_Q8_1_MMVQ>(...)
    break;
```

The dispatch in `ggml-cuda.cu` that decides whether to use MMVQ is:
```c
...
```
No custom function is needed — `ggml_is_quantized` returns true for all HIFI types.

> **Block stride bug (Q6_K_HIFI / Q6_K_HIFI_DYNAMIC)**: These types currently reuse
> `vec_dot_q6_K_q8_1` in `get_vec_dot_q_cuda`. That function advances the block pointer
> as `(const block_q6_K *) vbq + kbx`, using `sizeof(block_q6_K) = 210` bytes per step.
> Actual Q6_K_HIFI blocks are 222 bytes. All blocks after index 0 are read at the wrong
> address, producing garbage results. Each type needs its own `vec_dot_q6_k_hifi_q8_1`
> that casts to `(const block_q6_k_hifi *)` and reads the base Q6_K fields correctly.

#### 11.2.3 GPU Registration — Four Dispatch Points

`ggml_cuda_mul_mat` itself has **no** per-type list — it uses `ggml_is_quantized()`
and `ggml_cuda_should_use_mmq()` to select paths automatically. There are four
distinct places to register a new HIFI type:

**1. `ggml_backend_cuda_device_supports_op` (`ggml-cuda.cu`) — CRITICAL**

This is the single gate that tells the compute graph scheduler whether the CUDA
backend handles an op at all. Add each new HIFI type to the `GGML_OP_MUL_MAT` case:

```c
case GGML_TYPE_Q3_K_HIFI:
case GGML_TYPE_Q4_K_HIFI:
    // fall through with other quantized types
    return true;
```

**Without this entry, the type is not recognised as GPU-capable.** The tensor
scheduler then places *all* tensors — including activation and state tensors — on CPU,
even when `--ngl 0` is used, causing a ~26× inference slowdown.

Note: `GGML_OP_GET_ROWS` in this same function lists only basic types
(`F16, F32, Q4_0, Q5_0, Q8_0`, etc.). HIFI types are **not** in that list and
fall back to CPU for embedding lookups. This is acceptable since `token_embd`
tensors are typically F16/F32 even in HIFI models.

**2. `ggml_get_to_fp32_cuda` and `ggml_get_to_fp16_cuda` (`convert.cu`)**

These are used by the large-batch cuBLAS path (`ggml_cuda_op_mul_mat_cublas`)
and by type-conversion ops. Both must be registered or an `GGML_ASSERT(!=nullptr)`
will abort at runtime for any batch larger than `MMVQ_MAX_BATCH_SIZE`:

```c
// In ggml_get_to_fp32_cuda:
case GGML_TYPE_Q3_K_HIFI: return dequantize_row_q3_k_hifi_cuda;
// In ggml_get_to_fp16_cuda:
case GGML_TYPE_Q3_K_HIFI: return dequantize_row_q3_k_hifi_cuda;
```

**3. `ggml_cuda_should_use_mmq` (`mmq.cu`) — intentionally EXCLUDED**

This function decides whether to use the MMQ compact-kernel path for large batches.
Q3_K_HIFI and Q4_K_HIFI intentionally **do not** appear in this switch (comments say
"uses MMVQ/dequant path instead"). Do not add them — large-batch falls through to the
cuBLAS dequant path (point 2 above) which is correct.

Only add a new HIFI type here if it has a dedicated MMQ compact-copy kernel (as
Q5_K_HIFI_RES8 does).

**4. MMVQ registration** (see §11.2.2) — small-batch single-token inference.

**Dispatch summary** for Q3_K_HIFI / Q4_K_HIFI:
| Batch size | Path used |
|---|---|
| ≤ `MMVQ_MAX_BATCH_SIZE` | MMVQ via `vec_dot_q3_k_hifi_q8_1` |
| > `MMVQ_MAX_BATCH_SIZE` | cuBLAS + dequant via `dequantize_row_q3_k_hifi_cuda` |
| `GET_ROWS` op | CPU fallback (CUDA backend returns `false`) |

Other backends (Metal, Vulkan, SYCL) follow the same three-part pattern: dequant
kernel, MMVQ/vec_dot registration, op-support registration.
...
8. **`block_q3_k_hifi` has NO `outlier_count` field.** Only `block_q3_k_hifi_res8` has
   `outlier_count`. For `block_q3_k_hifi`, unused outlier slots use the sentinel pattern:
   `outlier_idx[k] = 255`, `outliers[k] = FP16(0.0f) = 0x0000`. The correct dequant
   pattern is a FP16-zero break:
   ```c
   // CPU (ggml_half = uint16_t):
   for (int k = 0; k < Q3_K_HIFI_OUTLIERS; ++k) {
       ggml_half oval = x[i].outliers[k];
       if (oval == 0) break;  // FP16-zero sentinel
       tmp[x[i].outlier_idx[k]] = GGML_FP16_TO_FP32(oval);
   }
   // CUDA/HIP (ggml_half = half):
   for (int k = 0; k < Q3_K_HIFI_OUTLIERS; ++k) {
       if (__half_as_ushort(x[i].outliers[k]) == 0) break;
       const int idx = x[i].outlier_idx[k];
       yb[idx] = __half2float(x[i].outliers[k]);
   }
   ```
   `Q4_K_HIFI` likewise has no `outlier_count` and uses the same FP16-zero sentinel.
...
15. **`block_q3_k_hifi` IS accessible inside CUDA/HIP translation units.**
    The `#if !defined(GGML_COMMON_DECL_CUDA)` guards in `ggml-common.h` wrap only the
    `#pragma pack` directives, not the struct `typedef`. All HIFI structs can be used
    directly via struct-pointer casts in GPU kernels — do not use byte-pointer arithmetic
    with hardcoded offsets.

    **Real pitfall**: `block_q3_k_hifi` (and `block_q4_k_hifi`) have **no**
    `outlier_count` field. Only `block_q3_k_hifi_res8` has `outlier_count`. Using
    `xb->outlier_count` on a `block_q3_k_hifi` pointer is a compile error.
    Use the FP16-zero sentinel instead: `__half_as_ushort(x[i].outliers[k]) == 0`
    (CUDA) or `xb->outliers[k] == 0` (CPU, where `ggml_half = uint16_t`).

    **Second pitfall**: The guard `if (idx >= 256) break` is always false when `idx`
    is promoted to `int` with value 255 (the sentinel). Use the FP16-zero check above.
