...
sentinel: break when the bit pattern of the FP16 value is `0x0000`.

> ### ⚠️ Critical Bug: Q4_K inner-loop output stride must be `+32`, not `+n`
>
> The standard `dequantize_block_q4_K` computes:
> ```c
> const int64_t n = 4;
> dst_t * y = yy + i*QK_K + 64*il + n*ir;
> y[l +  0] = d1 * (q[l] & 0xF) - m1;  // low nibble
> y[l + 32] = d2 * (q[l] >>  4) - m2;  // high nibble — hardcoded 32, NOT n
> ```
> When copying this pattern into the HIFI kernel, it is tempting to write `y[l + n]`
> for symmetry with the low-nibble line. **This is wrong.** `n = 4`; the high nibble
> must land 32 elements ahead of the thread's base, not 4. Using `y[l + n]` writes
> all high-nibble values to positions 4–7, leaves positions 32–255 uninitialised,
> and those uninitialised FP16/FP32 values propagate through matrix multiply to
> produce **-nan** for every perplexity chunk. There is no error message; the kernel
> launches and completes without CUDA errors.

Complete Q4_K_HIFI kernel:
```c
...
static __global__ void dequantize_block_q4_k_hifi(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    ...
    const block_q4_K * x = (const block_q4_K *) blk;  // first 144 bytes = base Q4_K
    ...
    // Thread 0 applies FP16 outlier replacements after all other threads finish
    __syncthreads();
    if (threadIdx.x == 0) {
...
**Q5_K_HIFI_RES8 CUDA dequant** — 64 threads (matching base Q5_K), followed by
thread-0 applying INT8 residuals scaled by a 1-byte E4M3 FP8 scale. Byte layout:
`[0..175]` = Q5_K base (176 bytes), `[176]` = outlier_count, `[177..184]` = idx[8],
`[185..192]` = residual_vals[8] (int8_t), `[193]` = residual_scale_e4m3, `[194..195]` = _reserved.

Implement the E4M3 → float decoder as a `__device__` function:
```c
__device__ static float e4m3fn_to_float(uint8_t b) {
...
Complete Q5_K_HIFI_RES8 kernel:
```c
...
static __global__ void dequantize_block_q5_k_hifi_res8(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    ...
    if (threadIdx.x == 0) {
        ...
        const float rscale = e4m3fn_to_float(blk[Q5_K_HIFI_RES8_GPU_E4M3_OFF]);
        dst_t * out = yy + i*QK_K;
        for (int k = 0; k < nc; k++) {
...
Add all four HIFI types to **both** `ggml_get_to_fp16_cuda` and `ggml_get_to_fp32_cuda`
dispatch switches in `convert.cu`:
```c
case GGML_TYPE_Q3_K_HIFI:     return dequantize_row_q3_k_hifi_cuda;
case GGML_TYPE_Q4_K_HIFI:     return dequantize_row_q4_k_hifi_cuda;
case GGML_TYPE_Q5_K_HIFI_RES8:return dequantize_row_q5_k_hifi_res8_cuda;
case GGML_TYPE_Q6_K_HIFI_RES8:return dequantize_row_q6_k_hifi_res8_cuda;
```
...
16. **Q4_K GPU kernel: high-nibble output stride is `+32`, not `+n`.** (See also the
    warning box in §11.2.1.)
    The `dequantize_block_q4_K` kernel unpacks the interleaved Q4_K nibbles using two
    separate `y[]` writes per thread:
    ```c
    const int64_t n = 4;
    dst_t * y = yy + i*QK_K + 64*il + n*ir;
    y[l +  0] = d1 * (q[l] & 0xF) - m1;  // low nibble
    y[l + 32] = d2 * (q[l] >>  4) - m2;  // high nibble — 32, not n
    ```
    Because `n` appears on the `y` base-pointer line immediately above, it is easy to
    write `y[l + n]` (= `y[l + 4]`) for the high nibble when porting to a new kernel.
    The kernel compiles and launches without error. The symptoms are:
    - Every perplexity chunk reports **-nan** (no finite values at all).
    - CUDA error checking reports success.
    - The model loads and the type is correctly dispatched.
    The root cause: high-nibble values are written to positions 4–7 (overwriting
    low-nibble output) while the correct positions 32–255 remain uninitialised in the
    output buffer. Those uninitialised FP16/FP32 values then propagate through the
    matrix multiply. There is no clue in the runtime output; the only fix is careful
    code review against the original `dequantize_block_q4_K` source.
    **Always use the hardcoded constant `32`, never the variable `n`.**

17. **Metal: `GGML_OP_GET_ROWS` must also exclude HIFI/LITE types.** In addition to
    `GGML_OP_MUL_MAT`, the `GGML_OP_GET_ROWS` case in `ggml_metal_device_supports_op`
    must return `false` for all 13 HIFI/LITE types. Without this, token embedding
    lookup for a HIFI-typed `token_embd.weight` tensor will be dispatched to Metal,
    which has no pipeline for it, and aborts with "not implemented":
    ```objc
    case GGML_OP_GET_ROWS:
        switch (op->src[0]->type) {
            case GGML_TYPE_Q3_K_HIFI: /* ... all 13 ... */ case GGML_TYPE_Q6_K_LITE:
                return false;
            default: break;
        }
        return true;
...
## 17. Build and Verification Checklist (Q3 / Q4 / Q5 HIFI)

This section provides a practical step-by-step checklist for porting or verifying the
three main HIFI quantization tiers. Complete steps in order; each gate catches a class
of bugs before the next phase.

### Phase 1 — Type Registration

- [ ] `ggml_type` enum contains all 13 HIFI/LITE IDs (42–54) in `ggml.h`.
- [ ] `llama_ftype` enum contains all 9 HIFI/LITE ftype IDs (44–52) in `llama.h`.
- [ ] `ggml.c` type-traits table: 13 entries, each with correct `type_size`,
      `to_float`, `from_float_ref`, `is_quantized = true`, `blck_size = 256`.
- [ ] `ggml.c` `ggml_quantize_chunk` switch: 13 cases, each dispatching to the
      correct `quantize_q*` function.
- [ ] `ggml_validate_row_data` switch: 13 HIFI/LITE cases in the "nothing to
      validate" arm (§15, note 13). Missing cases abort quantization silently.
- [ ] `llama-model-loader.cpp` `type_max → ftype` map: entries for Q3_K_HIFI,
      Q4_K_HIFI, Q5_K_HIFI_RES8, Q6_K_HIFI_RES8, all LITE types.
- [ ] `tools/quantize/quantize.cpp` type table: `Q3_K_HIFI`, `Q4_K_HIFI`,
      `Q5_K_HIFI` entries appear in `--help` output.

**Verification:**
```sh
./build/bin/llama-quantize --help | grep -E "HIFI|LITE"
# Expected: Q2_K_HIFI through Q6_K_LITE all listed
```

### Phase 2 — Quantization (CPU only)

- [ ] `ggml-quants-hifi.c` included in `ggml/src/CMakeLists.txt`.
- [ ] `llama-quant.cpp` includes `#define GGML_COMMON_DECL_CPP` before
      `ggml-common.h` so that `Q3_K_HIFI_MAX_OUTLIERS` etc. are visible.
- [ ] `llama-quant.cpp` uses `vocab.n_tokens()` (not `n_vocab()`) for model
      parameter estimation.

**Q3_K_HIFI quantize test:**
```sh
./build/bin/llama-quantize <model.f16.gguf> <out.gguf> Q3_K_HIFI
# Expect: lines showing "converting to Q3_K_HIFI" and/or "converting to Q6_K_HIFI"
# for OUTPUT / TOKEN_EMBD on models ≥1.7B.
# Check the final BPW is ~3.7–4.3, not 4.84 (Q3_K_M).
```

**Q4_K_HIFI quantize test:**
```sh
./build/bin/llama-quantize <model.f16.gguf> <out.gguf> Q4_K_HIFI
# Expect: "converting to Q4_K_HIFI" for most tensors,
# "converting to Q5_K_HIFI_RES8" for token_embd, output, and early attn_qkv/attn_v.
# Final BPW ~4.9–5.3.
```

**Q5_K_HIFI quantize test:**
```sh
./build/bin/llama-quantize <model.f16.gguf> <out.gguf> Q5_K_HIFI
# Expect: "converting to Q5_K_HIFI_RES8" or "converting to Q6_K_HIFI_RES8"
# for critical tensors; rest Q5_K.
```

### Phase 3 — CPU Inference (no GPU)

These tests isolate CPU-path bugs before involving any GPU backend.

- [ ] `ggml-cpu/ggml-cpu.c` `type_traits_cpu[]`: 13 entries with correct
      `vec_dot`, `vec_dot_type = GGML_TYPE_Q8_K`, `nrows = 1`.
- [ ] `ggml-cpu/ops.cpp` `ggml_compute_forward_clamp`: 13 HIFI/LITE cases in
      the `GGML_ABORT` arm (suppresses `-Wswitch`).
- [ ] `ggml-cpu/ops.cpp` `ggml_compute_forward_get_rows`: 13 HIFI/LITE cases in
      the `ggml_compute_forward_get_rows_q` arm. **Required** if any HIFI type is
      used for `token_embd.weight` (Q4_K_HIFI maps it to `Q5_K_HIFI_RES8`).

**CPU perplexity smoke test:**
```sh
./build/bin/llama-perplexity \
    -m <out_Q4_K_HIFI.gguf> \
    -f <wiki.test.raw> \
    -c 512 --chunks 3 --no-kv-offload
# All chunks must produce finite perplexity (e.g. 18–22 for a 0.8B model).
# A single -nan chunk indicates a NaN in the compute graph.
```

If `-nan` appears on CPU:
1. Check `ggml_compute_forward_get_rows` has all 13 HIFI/LITE cases — a missing
   case causes a hard abort that manifests as NaN propagation in some builds.
2. Check the `ggml_vec_dot_q4_k_hifi_q8_K` sentinel loop: `if (oval == 0) break`
   must use the FP16 bit pattern check, not `GGML_FP16_TO_FP32(oval) == 0.0f`.
3. Verify `quantize_row_q4_K_ref` was called with `QK_K` (256) floats, not
   `n_per_row` (which may be much larger).

### Phase 4 — Metal Fallback (macOS)

- [ ] `ggml-metal-device.m` `GGML_OP_MUL_MAT` / `GGML_OP_MUL_MAT_ID`: all 13
      HIFI/LITE types return `false` before the `has_simdgroup_reduction` check.
- [ ] `ggml-metal-device.m` `GGML_OP_GET_ROWS`: all 13 types return `false`
      (§15, note 17). Without this, a HIFI `token_embd.weight` aborts at
      inference time with "not implemented".

**Metal smoke test (macOS only):**
```sh
./build/bin/llama-perplexity \
    -m <out_Q4_K_HIFI.gguf> \
    -f <wiki.test.raw> \
    -c 512 --chunks 3
# Should produce finite perplexity; Metal handles standard Q4_K/Q6_K tensors,
# CPU handles HIFI tensors. No abort.
```

### Phase 5 — CUDA/ROCm GPU Inference

- [ ] `mmvq.cu` `ggml_cuda_has_mmvq_kernel`: returns `false` for all 13
      HIFI/LITE types (no native vec_dot kernel registered).
- [ ] `ggml-cuda.cu` `use_mul_mat_vec_q` check: guarded with
      `&& ggml_cuda_has_mmvq_kernel(src0->type)`.
- [ ] `ggml-cuda.cu` supported-types list: all 13 HIFI/LITE types present in
      the `GGML_OP_MUL_MAT` block. **Critical for tensor placement** (§15, note 14).
- [ ] `convert.cu` dequant kernels: all four HIFI types registered in both
      `ggml_get_to_fp16_cuda` and `ggml_get_to_fp32_cuda`.

**Q4_K_HIFI CUDA kernel review checklist** (most likely source of GPU-only NaN):

| Check | Correct | Wrong (produces NaN) |
|-------|---------|----------------------|
| High-nibble write | `y[l + 32]` | `y[l + n]` (n=4) |
| Thread count | `<<<nb, 32>>>` | `<<<nb, 64>>>` |
| Sentinel check | `__half_as_ushort(v) == 0` | `__half2float(v) == 0.0f` only |
| Sync before thread-0 | `__syncthreads()` before `if (threadIdx.x == 0)` | Missing sync |

**GPU perplexity smoke test:**
```sh
./build/bin/llama-perplexity \
    -m <out_Q4_K_HIFI.gguf> \
    -f <wiki.test.raw> \
    -c 512 --chunks 3 -ngl 99
# All chunks must be finite. -nan with -ngl but not without confirms a GPU
# dequant kernel bug (most likely the y[l + 32] issue above).
```

If GPU produces -nan but CPU does not:
1. Verify `y[l + 32]` in `dequantize_block_q4_k_hifi` — this is the most
   common cause, confirmed in the original port.
2. Verify the E4M3 decoder in `dequantize_block_q5_k_hifi_res8` handles the
   NaN bit pattern `0x7F` → returns `0.0f` rather than propagating NaN.
3. Check that `Q5_K_HIFI_RES8_GPU_STRIDE = 196` matches `sizeof(block_q5_k_hifi_res8)`.

### Phase 6 — Quality Regression Check

Run perplexity on all three tiers and compare to baseline Q-type at similar BPW:

| Quantization | Expected PPL vs baseline |
|---|---|
| Q3_K_HIFI (≥3B model) | Better than Q3_K_M at lower BPW |
| Q4_K_HIFI | Better than Q4_K_M at ~0.3 BPW overhead |
| Q5_K_HIFI | Better than Q5_K_M at ~0.2 BPW overhead |

If HIFI PPL is **worse** than the standard equivalent:
- Check that outlier positions are actually zeroed before the base quantizer call
  (not just marked for later correction). Failing to zero means the base block
  scale is dominated by the outlier values, reducing accuracy for all other weights.
- For Q4_K_HIFI: verify the sentinel loop stops at the first `outliers[k] == 0`
  slot (FP16 bit pattern zero). If all 8 slots are applied unconditionally, position
  0 gets overwritten with 0.0 every block.
- For Q5_K_HIFI_RES8: verify `residual_scale` is decoded correctly on CPU
  (`e4m3_to_float`) and GPU (`e4m3fn_to_float`). A scale of 0 makes all
  residuals multiply to 0, silently disabling the correction.
