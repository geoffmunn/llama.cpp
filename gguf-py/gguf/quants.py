...
class Q3_K_HIFI(__Quant, qtype=GGMLQuantizationType.Q3_K_HIFI):
    ...
    def dequantize_blocks(cls, blocks: np.ndarray) -> np.ndarray:
        ...
        # Q3_K_HIFI structure: Q3_K base (110 bytes) + extension (27 bytes, optimized from 50)
        # Base: hmask[32] + qs[64] + scales[12] + d[2] = 110 bytes
        # Extension: n_outliers[1] + outlier_idx[8] + _align_pad[1] + outlier_vals[16] + padding[1] = 27 bytes (8 outliers, optimized from 16)
        ...
        # Extract outlier data (8 outliers instead of 16)
        outlier_count = blocks[:, base_size:base_size+1].astype(np.uint8)
        outlier_idx = blocks[:, base_size+1:base_size+9].astype(np.uint8)  # 8 bytes
        outlier_vals = blocks[:, base_size+10:base_size+26].view(np.float16).astype(np.float32)  # 8 FP16 values = 16 bytes (skip _align_pad at offset 9)
        
        # Apply outlier corrections
        result = q3k_result.copy()
        for i in range(n_blocks):
            n_outliers = min(int(outlier_count[i, 0]), 8)  # Max 8 outliers
            for k in range(n_outliers):
                idx = int(outlier_idx[i, k])
                if idx < QK_K:
                    result[i, idx] = float(outlier_vals[i, k])  # Replace, not add (as per C implementation)
