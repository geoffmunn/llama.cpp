...
void quantize_row_q2_k_hifi_ref(const float * GGML_RESTRICT x, block_q2_k_hifi * GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; ++i) {
        const float * xb = x + i * Q2_K_HIFI_BLOCK_SIZE;

        // Step 1: Quantize with standard Q2_K first
        quantize_row_q2_K_ref(xb, (block_q2_K *)(&y[i]), Q2_K_HIFI_BLOCK_SIZE);

        // Step 2: Dequantize to get base approximation
        float tmp[Q2_K_HIFI_BLOCK_SIZE];
        dequantize_row_q2_K((const block_q2_K *)(&y[i]), tmp, Q2_K_HIFI_BLOCK_SIZE);

        // Step 3: Find top outliers by absolute residual
        struct outlier_candidate {
            int idx;
            float err;
        } candidates[Q2_K_HIFI_BLOCK_SIZE];

        for (int j = 0; j < Q2_K_HIFI_BLOCK_SIZE; ++j) {
            candidates[j].idx = j;
            candidates[j].err = fabsf(xb[j] - tmp[j]);
        }

        // Partial sort to find top Q2_K_HIFI_MAX_OUTLIERS
        for (int o = 0; o < Q2_K_HIFI_MAX_OUTLIERS; ++o) {
            for (int j = o + 1; j < Q2_K_HIFI_BLOCK_SIZE; ++j) {
        ...
        float xb_copy[Q2_K_HIFI_BLOCK_SIZE];
        memcpy(xb_copy, xb, Q2_K_HIFI_BLOCK_SIZE * sizeof(float));
        for (int o = 0; o < Q2_K_HIFI_MAX_OUTLIERS; ++o) {
            xb_copy[candidates[o].idx] = 0.0f;
        }
        quantize_row_q2_K_ref(xb_copy, (block_q2_K *)(&y[i]), Q2_K_HIFI_BLOCK_SIZE);

        // Step 5: Store outlier info (sorted ascending by index)
        y[i].outlier_count = (uint8_t)Q2_K_HIFI_MAX_OUTLIERS;
        for (int o = 0; o < Q2_K_HIFI_MAX_OUTLIERS; ++o) {
            int best = o;
            for (int j = o + 1; j < Q2_K_HIFI_MAX_OUTLIERS; ++j) {
            ...
            y[i].outlier_vals[o] = GGML_FP32_TO_FP16(xb[candidates[o].idx]);
        }
...
void dequantize_row_q2_k_hifi(const block_q2_k_hifi * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; ++i) {
        // Dequantize base Q2_K
        dequantize_row_q2_K((const block_q2_K *)(&x[i]), y + i * Q2_K_HIFI_BLOCK_SIZE, Q2_K_HIFI_BLOCK_SIZE);

        // Replace outlier positions with FP16 values
        const int count = x[i].outlier_count;
        for (int o = 0; o < count; ++o) {
            y[i * Q2_K_HIFI_BLOCK_SIZE + x[i].outlier_idx[o]] = GGML_FP16_TO_FP32(x[i].outlier_vals[o]);
        }
    }
}

// ====================== Q3_K_HIFI (de)-quantization
...
void quantize_row_q3_k_hifi_ref(const float * GGML_RESTRICT x, block_q3_k_hifi * GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        struct outlier_candidate {
            int idx;
            float err;
        } candidates[Q3_K_HIFI_BLOCK_SIZE];
        ...
        // Partial sort to find top Q3_K_HIFI_OUTLIERS
        for (int o = 0; o < Q3_K_HIFI_OUTLIERS; ++o) {
            for (int j = o + 1; j < Q3_K_HIFI_BLOCK_SIZE; ++j) {
                if (candidates[j].err > candidates[o].err) {
                    float tmp_err = candidates[o].err;
                    candidates[o].err = candidates[j].err;
                    candidates[j].err = tmp_err;
        ...
        // Step 4: Zero outlier positions and re-quantize base
        float xb_copy[Q3_K_HIFI_BLOCK_SIZE];
        memcpy(xb_copy, xb, Q3_K_HIFI_BLOCK_SIZE * sizeof(float));
        for (int o = 0; o < Q3_K_HIFI_OUTLIERS; ++o) {
            xb_copy[candidates[o].idx] = 0.0f;
        }
        ...
        for (int o = 0; o < Q3_K_HIFI_OUTLIERS; ++o) {
            // Sort by index for compact storage
            int best = o;
            for (int j = o + 1; j < Q3_K_HIFI_OUTLIERS; ++j) {
                if (candidates[j].idx < candidates[best].idx) {
                    best = j;
                }
            }
...
void dequantize_row_q3_k_hifi(const block_q3_k_hifi * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const int count = x[i].outlier_count;
        for (int o = 0; o < count; ++o) {
            y[i * Q3_K_HIFI_BLOCK_SIZE + x[i].outlier_idx[o]] = GGML_FP16_TO_FP32(x[i].outliers[o]);
        }
    }
}

size_t quantize_q3_k_hifi(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * GGML_RESTRICT quant_weights) {
    size_t row_size = n_per_row / Q3_K_HIFI_BLOCK_SIZE * sizeof(block_q3_k_hifi);
    if (!quant_weights) {
        quantize_row_q3_k_hifi_ref(src, (block_q3_k_hifi *)dst, nrow * n_per_row);
    } else {
...
size_t quantize_q4klite(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * GGML_RESTRICT quant_weights) {
    // Q4K Lite uses the same block layout as Q4_K for the base quantization
    size_t row_size = n_per_row / 256 * sizeof(block_q4_K);
    if (!quant_weights) {
        quantize_row_q4_K_ref(src, (block_q4_K *)dst, nrow * n_per_row);
    } else {
        // TODO: imatrix-guided quantization
        quantize_row_q4_K_ref(src, (block_q4_K *)dst, nrow * n_per_row);
    }
...
size_t quantize_q5klite(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * GGML_RESTRICT quant_weights) {
    // Q5K Lite uses a Q4_K-like base layout with INT8 residual extension
    size_t row_size = n_per_row / 256 * sizeof(block_q5_k_lite);
    if (!quant_weights) {
        // Zero-extend the extra INT8 fields per block
        block_q5_k_lite * dst_block = (block_q5_k_lite *)dst;
