...
void dequantize_q2_k_hifi(device const block_q2_k_hifi * xb, short il, thread type4x4 & reg) {
    // Q2_K_HIFI uses Q2_K-compatible layout: scales[16] + qs[64] + dm + outliers
...
// Q2_K_HIFI: Q2_K with 16 FP16 outlier corrections
// Reuses Q2_K kernel logic and adds outlier corrections
template<int nr0, typename args_t>
void kernel_mul_mv_q2_k_hifi_f32_impl(
    ...
    device const float * y4 = y + ix * QK_K + 128 * iq + 8 * ir;
    const int y_offset = ix * QK_K + 128 * iq + 8 * ir;
    ...
    for (int i = ix; i < nb; i += 4) {
        for (short row = 0; row < nr0; ++row) {
            device const block_q2_k_hifi * xb = x + i + row * (args.nb01 / sizeof(block_q2_k_hifi));
