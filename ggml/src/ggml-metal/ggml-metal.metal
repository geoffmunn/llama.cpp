...
void dequantize_q2_k_hifi(device const block_q2_k_hifi *xb, short il, thread type4x4 & reg) {
    ...
    for (int k = 0; k < n_outliers; ++k) {
        const int outlier_idx = (int)xb->outlier_idx[k];
...
void kernel_mul_mv_q2_k_hifi_f32_impl(
    ...
    for (int ib = ix; ib < nb; ib += 4) {
        ...
        for (short row = 0; row < nr0; row++) {
            // Calculate exact byte offset for this block
            // Row offset: row * args.nb01 (where nb01 = number_of_blocks_per_row * 122)
            // Block offset within row: ib * 122
            device const block_q2_k_hifi * xb = (device const block_q2_k_hifi *)(src0_base + row * args.nb01 + ib * sizeof(block_q2_k_hifi));
            
            device const uint8_t  * sc = xb->scales + 8*iq + is;
            device const uint16_t * qs = (device const uint16_t *)xb->qs + 16 * iq + 4 * ir;
